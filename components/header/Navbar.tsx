      ...
      {navigation.map((path) => {
        return (
          <div key={path.href}>
            ...
            <DropdownMenuItem className="p-2 hover:bg-white hover:text-blue-700">
              <Link
                href={path.href}
                className="p-2 focus:outline-none focus:bg-white focus:text-blue-700"
              >
                Explore {path.title}
              </Link>
            </DropdownMenuItem>
            {path.routes &&
              path.routes.map((route) => {
                return (
                  <DropdownMenuItem
                    key={route.href}
                    className="p-2 hover:bg-white hover:text-blue-700"
                  >
                    <Link
                      href={route.href}
                      className="p-2 focus:outline-none focus:bg-white focus:text-blue-700"
  ...
  return (
    <NavigationMenu.Content className="absolute top-[100%] z-50 w-max rounded-md shadow-md">
      <ul className="list-none p-4 m-0 bg-neutral-50 flex flex-col gap-2">
        <li className="p-2 hover:bg-white hover:text-blue-700">
          <NavigationMenu.Link
            href={parentHref}
            className="p-2 focus:outline-none focus:bg-white focus:text-blue-700"
          >
