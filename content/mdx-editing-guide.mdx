...
The `<SectionHeader/>` component is used to create a header for the page. It is a custom component that is defined in `/components/section-header.tsx` and imported in `/mdx-components.tsx`.

This is the main content of the page. You can write normal text here. You can use **bold** and *italic* formatting.
...
Insert a link to a markdown cheat sheet using the `<LinkButton/>` component. `<LinkButton/>` uses the same styling as the buttons that are used throughout this website.
