...
export default async function ClassroomSupport() {
  const sectionData = await readContentFolder<ClassroomSectionData>(
    "content/services/classroom"
  )
  ...
  }>("content/services/classroom/featured-carousel.yaml")
  ...
  return (
    <div>
      ...
      <section className="content-wrapper pt-24 px-14 lg:px-36">
        ...
        <Card className="w-full border-none shadow-none rounded-none">
          <CardContent className="mx-auto flex flex-col px-6">
            ...
            {/* Buttons */}
            {inClassTutorials.data.links &&
              inClassTutorials.data.links.filter(
      ...
      <section className="content-wrapper py-24 px-14 lg:px-36 bg-neutral-50">
        ...
        <Card className="w-full border-none shadow-none rounded-none">
          <CardContent className="mx-auto flex flex-col px-6">
            ...
            {/* Buttons */}
            {studentAccounts.data.links &&
              studentAccounts.data.links.filter(
                (link) => link.category === "Default"
              ).length > 0 && (
                <div className="flex gap-4 mt-6 not-prose">
                  {studentAccounts.data.links
      ...
      <section className="content-wrapper py-24 px-14 lg:px-36">
        <SectionHeader
          title={computationalNotebooks.data.title}
          align="center"
        />
        <Card className="w-full border-none shadow-none rounded-none">
          <CardContent className="mx-auto flex flex-col px-6">
            <div className="prose text-xl">
              ...
              {(computationalNotebooks.data as YamlContentData).description && (
                ...
                >
                  {
                    (computationalNotebooks.data as YamlContentData)
                      .description!
                  }
                </Markdown>
              )}
              ...
              {/* Sections */}
              {(computationalNotebooks.data as YamlContentData).sections?.map(
                (section) => (
                  <div key={section.name}>
                    ...
                    {section.links && section.links.length > 0 && (
                      <div className="flex gap-4 mt-6 not-prose">
                        {section.links.map(
                          (link: ContentLinks, linkIndex: number) => (
                            <ButtonLink
                              href={link.target}
                              key={`${section.name}-${linkIndex}`}
                              variant={
                                section.name === "google-colab"
                                  ? "primary_filled"
                                  : "secondary_filled"
                              }
                              size="lg"
